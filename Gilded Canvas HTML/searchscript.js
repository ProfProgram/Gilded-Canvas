...
function displayProducts() {
    document.getElementById("products").innerHTML = ""; // Clear previous products to avoid duplication
    for (let i of products.data) {
        let card = document.createElement("div");
        card.classList.add("card", i.category.replace(/\s+/g, "-"), "hide");
...
document.getElementById("search").addEventListener("click", () => {
    ...
    let elements = document.querySelectorAll(".product-name"); // Select all product name elements
    let cards = document.querySelectorAll(".card"); // Select all cards
    // Loop through all elements
    elements.forEach((element, index) => {
        // Convert product name to lowercase and check if it includes the search input
